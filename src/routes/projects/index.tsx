...
function Projects() {
  ...
  return (
    <Section
      ...
      description="13+ Flutter apps shipped — 5 live on the App Store and Google Play — across social commerce, beauty, ride-sharing, agri-tech, fintech, edtech and government sectors."
    >
      <div className="grid gap-6 md:grid-cols-2">
        {projects.map((p) => (
          ...
          >
            <div
              className={`absolute inset-x-0 top-0 z-10 h-px bg-gradient-to-r ${p.accent} opacity-60`}
            />
            <Link
              to="/projects/$slug"
              params={{ slug: p.slug }}
              ...
              {p.images[0] && (
                <div className="aspect-video w-full overflow-hidden bg-secondary/30">
                  <img
                    ...
                    loading="lazy"
                    decoding="async"
                    className="h-full w-full object-cover object-top transition-spring group-hover:scale-105"
                  />
                </div>
              )}
